import numpy as np
from Model.activation.relu import ReLU
...
class BiasLayer:
    ...
    def __init__(self, input_layer, activation=None) -> None:
        ...
        _, num_in_features = input_layer.output_dimension
        self.W = np.random.randn(1, num_in_features) / np.sqrt(num_in_features)
    ...
    def backward(self, downstream):
        ...
        if self.activation != None:
            ...
            self.input_layer.backward(self.activation.backward(downstream, self.activated_output))
        else:
